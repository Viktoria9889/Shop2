...
const addToBasket = async (req, res, next) => {
    try {
        ...
        const basketDevice = await BasketDevice.create({ device_id, basket_id: basket.id, quantity: quantity || 1 })
...
const removeFromBasket = async (req, res, next) => {
    try {
        ...
        if (device_id.length === 1) {
            const deleteResult = await BasketDevice.deleteOne({ basket_id: basket.id, device_id: device_id[0] })
