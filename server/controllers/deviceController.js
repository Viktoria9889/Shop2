...
const create = async (req, res, next) => {
    try {
        ...
        if (parsedInfo) {
            const createdDeviceInfo = await Promise.all(parsedInfo.map(async i =>
...
const getAll = async (req, res) => {
    ...
    if (!brandId && !typeId) {
        devices = await Device.find().limit(Number(limit)).skip(offset)
            .populate('typeId')
            .populate('brandId')
        totalDevicesCount = await Device.countDocuments()
    } else if (brandId && !typeId) {
        devices = await Device.find({ brandId }).limit(Number(limit)).skip(offset)
            .populate('typeId')
            .populate('brandId')
        totalDevicesCount = await Device.countDocuments({ brandId })
    } else if (!brandId && typeId) {
        devices = await Device.find({ typeId }).limit(Number(limit)).skip(offset)
            .populate('typeId')
            .populate('brandId')
        totalDevicesCount = await Device.countDocuments({ typeId })
    } else if (brandId && typeId) {
        devices = await Device.find({ brandId, typeId }).limit(Number(limit)).skip(offset)
            .populate('typeId')
            .populate('brandId')
