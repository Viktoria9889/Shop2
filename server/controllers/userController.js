...
const registration = async (req, res, next) => {
    ...
    try {
       ...
       // console.log('Registration user:', user)
        ...
        // Зберігання токена
        const token = await tokenController.saveToken({ user: user.id, email: user.email, role: user.role })
...
const login = async (req, res, next) => {
    ...
    const token = await tokenController.saveToken({ user: user.id, email: user.email, role: user.role }) // Зберігаємо токени при вході
